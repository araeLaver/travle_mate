...
**앱 콘텐츠 선언 — 10건 중 3건 완료 (2026-09-19)**
...
| 로그인 세부정보 | ⬜ | **예**(로그인 필수) + 검토용 계정 필요. 라이브 서버에서 동작 확인된 계정: `qa-check-20260913@example.com`. **비밀번호 입력이 필요해 자동화로 처리하지 않음** |
| 금융 기능 | ⬜ | 대부분 해당 없음이나 **"토큰화된 디지털 애셋(NFT) 판매·거래·보상"** 체크 여부가 판단 필요 — 앱 UI엔 NFT가 있으나 프로덕션에서 온체인 민팅은 꺼져 있음(`BLOCKCHAIN_REQUIRE_IN_PROD=false`) |
| 콘텐츠 등급 / 타겟층 / 데이터 보안 | ⬜ | 앱 동작에 대한 법적 진술이라 개발자 본인이 답할 것 |

**접근 경로(중요)**: `앱 콘텐츠` 인덱스는 **`/app-content/overview`**다. 좌측 네비게이션은 클릭해도 라우팅되지 않고
`/app-content` 단독 URL은 앱 목록으로 튕긴다. 각 항목은 `/app-content/<slug>` — 확인된 슬러그:
`privacy-policy`, `ads-declaration`, `testing-credentials`(로그인 세부정보), `government-apps`, `finance`(금융 기능), `health`.
**Play Console의 라디오·체크박스·버튼은 좌표 클릭이 거의 통하지 않는다.** `find`로 ref를 얻어 클릭할 것.
