...
describe('NotificationCenter', () => {
  ...
  test('알림 클릭 시 읽음 처리 및 페이지 이동', async () => {
    ...
    // 첫 번째 알림 클릭 (읽지 않음)
    await waitFor(() => {
      const notification = screen.getByText('그룹 초대');
      fireEvent.click(notification.closest('.notification-item')!);
    });
  ...
  test('모두 읽음 버튼 클릭 테스트', async () => {
    ...
    // 모두 읽음 버튼 클릭
    await waitFor(() => {
      const markAllReadButton = screen.getByRole('button', { name: /모두 읽음/i });
