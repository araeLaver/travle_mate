...
describe('SearchBar', () => {
  ...
  test('자동완성 항목 클릭 시 페이지 이동', async () => {
    ...
    const suggestion = await screen.findByText('제주도 여행');
    fireEvent.click(suggestion);

    expect(mockNavigate).toHaveBeenCalledWith('/search?q=' + encodeURIComponent('제주도 여행'));
  });
  ...
  test('검색 버튼 클릭', () => {
    ...
    expect(mockNavigate).toHaveBeenCalledWith('/search?q=' + encodeURIComponent('서울'));
  });
