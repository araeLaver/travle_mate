...
export function OfflineNotice({ className = '' }: OfflineNoticeProps) {
  ...
  if (showReconnected) {
    return (
      <div
        className={`fixed top-0 left-0 right-0 z-50 bg-success text-white px-4 py-3 shadow-lg animate-slide-down ${className}`}
        role="alert"
      >
        <div className="flex items-center justify-center gap-2 max-w-7xl mx-auto">
...
export function OfflinePage() {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-sand-100 dark:bg-gray-900 px-4">
      <div className="text-center max-w-md">
        ...
        <div className="space-y-4">
          <button
            ...
            className="w-full px-6 py-3 bg-primary-500 text-white font-bold rounded-[13px] hover:bg-primary-700 transition-colors"
          >
            {t('common.retry')}
          </button>
...
export function UpdateNotice({ onUpdate, onDismiss }: UpdateNoticeProps) {
  return (
    <div
      className="fixed bottom-4 left-4 right-4 md:left-auto md:right-4 md:w-96 z-50 bg-ink text-white rounded-2xl shadow-[0_10px_30px_rgba(16,16,20,0.25)] p-4"
      role="alert"
    >
      <div className="flex items-start gap-3">
        <div className="flex-1">
          <h3 className="font-semibold">새 버전이 있습니다</h3>
          <p className="text-sm text-[#A0A0AC] mt-1">업데이트하여 최신 기능을 사용하세요.</p>
        </div>
        <button onClick={onDismiss} className="p-1 hover:bg-white/10 rounded" aria-label="닫기">
      ...
      <div className="flex gap-2 mt-4">
        <button
          onClick={onUpdate}
          className="flex-1 px-4 py-2 bg-[#8E7BFF] text-[#0C0C11] font-extrabold rounded-[11px] hover:bg-[#B3ABFF] transition-colors"
        >
          지금 업데이트
        </button>
        <button
          onClick={onDismiss}
          className="px-4 py-2 text-[#A0A0AC] hover:text-white transition-colors"
        >
          나중에
        </button>
      </div>
