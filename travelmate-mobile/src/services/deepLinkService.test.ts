...
describe('deepLinkService parsing', () => {
  it('parses supported custom scheme and universal links', () => {
    expect(parseDeepLink('doorimate://location/42')).toEqual({
      screen: 'LocationDetail',
      params: { locationId: 42 },
    });
    expect(parseDeepLink('https://doorimate.com/user/77')).toEqual({
      screen: 'UserProfile',
      params: { userId: 77 },
    });
    expect(parseDeepLink('doorimate://chat/9?name=서울%20채팅')).toEqual({
      screen: 'ChatRoom',
  ...
  it('parses review links with match and target user ids', () => {
    expect(parseDeepLink('https://doorimate.com/review/15/88')).toEqual({
      screen: 'Review',
  ...
  it('rejects partial, missing, zero, and negative numeric ids', () => {
    expect(parseDeepLink('doorimate://location/42abc')).toBeNull();
    expect(parseDeepLink('doorimate://location/0')).toBeNull();
    expect(parseDeepLink('doorimate://chat/-9')).toBeNull();
    expect(parseDeepLink('https://doorimate.com/review/15')).toBeNull();
    expect(parseDeepLink('https://doorimate.com/user/1.5')).toBeNull();
  });
  ...
  it('navigates parsed deep links only when navigation is ready', () => {
    ...
    expect(handleDeepLinkUrl('doorimate://chat/9?name=서울%20채팅')).toBe(true);
    ...
    expect(handleDeepLinkUrl('doorimate://chat/9')).toBe(false);
  ...
  it('does not navigate invalid or empty deep links', () => {
    ...
    expect(handleDeepLinkUrl('doorimate://location/42abc')).toBe(false);
    expect(navigate).not.toHaveBeenCalled();
  });
});
